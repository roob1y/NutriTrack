...
function WeeklyCalChart({ weekDates, mealLog, target }) {
  ...
  const max = Math.max(target * 1.2, ...data, 1);
          ...
          <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <div
              style={{
                ...
                background: isToday ? 'var(--accent)' : over ? 'var(--red)' : 'var(--border)',
                borderRadius: '4px 4px 0 0',
                transition: 'height 0.4s ease',
              }}
            />
            <div style={{ fontSize: '10px', color: isToday ? 'var(--accent)' : 'var(--muted)', fontWeight: 600 }}>
              {DAY_LABELS[i]}
            </div>
          </div>
        );
...
export default function ProgressView() {
  ...
  const avgCals = daysLogged ? Math.round(weekTotals.reduce((a, t) => a + t.calories, 0) / daysLogged) : 0;
  const avgProtein = daysLogged ? Math.round(weekTotals.reduce((a, t) => a + t.protein, 0) / daysLogged) : 0;

  const todayTotals = sumMacros(mealLog[today] || []);
  const daysOnTarget = weekTotals.filter((t) => t.calories > 0 && t.calories <= targets.calories).length;
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 600, marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          Daily calories vs {targets.calories} kcal goal
        </div>
        <WeeklyCalChart weekDates={weekDates} mealLog={mealLog} target={targets.calories} />
        {/* Target line label */}
        <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '8px', textAlign: 'right' }}>
          🟠 = today · 🔴 = over goal
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        {[
          { label: 'Calories', val: Math.round(todayTotals.calories), unit: 'kcal', target: targets.calories },
          { label: 'Protein',  val: Math.round(todayTotals.protein),  unit: 'g',    target: targets.protein },
          ...
          return (
            ...
            >
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <span style={{ fontSize: '12px', color: 'var(--muted)', marginLeft: '4px' }}>/ {target}{unit}</span>
              </div>
            </div>
          );
        })}
        <div style={{ fontSize: '12px', color: 'var(--muted)', textAlign: 'center', marginTop: '-4px' }}>
          Last entry: {mealLog[today]?.length ? `${mealLog[today].length} meals logged` : 'Nothing logged today'}
        </div>
      </div>
    </div>
  );
